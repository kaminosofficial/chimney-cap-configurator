...
1. **Snapshots the existing cart lines first** (`saveCartSnapshotBeforeClear` → sessionStorage `chimney-cap-cart-snapshot`: variant id, quantity, line properties; 60-min expiry)
...
- 3D canvas → JPEG (white-background composite) via `captureCanvasScreenshot()`. The capture frames the camera with `cameraActions.fitView()` (bounding-sphere fit, 18% margin) so the WHOLE product fits at a consistent size, and lands inside Shopify's centered-square cart tile — **no pixel-crop/pad needed for the cart**. `snapshot()`/`restore()` put the user's live camera back after.
- `canvasToJpegUnderLimit()` downscales (>1400px) + steps quality down so the payload stays under the `variant-image` ~525KB/700k-base64 limit (413 isn't retried → would leave the variant imageless → default photo).
- Cart/buy capture is deliberately LIGHT (fitView + composite + size guard, NO getImageData crop); the PDF passes `{ framed: true }` → additionally content-crops tight for the hero. See the **session-2 Key Decisions** entry.
- Uploaded via `POST /api/variant-image`, retried by `uploadVariantImageWithRetry` (3×/2×; 413 not retried). Reuse-heal re-uploads when a reused variant reports `variantHasImage:false`.
- Add to Cart: capture + upload run in parallel with the add; the drawer opens once price is verified, image fills in via the post-open refresh. Buy Now `await`s the upload before the `/checkout` redirect.

---

## PDF Generation

Users preview + download an A4 spec sheet via the **Export PDF** button (`Sidebar.tsx` → `PdfPreviewModal`).

### How It Works

1. `PdfPreviewModal.tsx` (lazy-loaded) captures a 3D snapshot (`{ framed: true }` → big, content-cropped hero), then renders `PdfReport.tsx` — the "Airy" layout: Kaminos header (cropped logo via `getCroppedLogo()`), **big hero** (`HERO_MAX_W=640, HERO_MAX_H=350`), and the cap spec (mount/lid/skirt/material) in two columns with **Pricing & Summary pinned to the bottom-right corner** (spec-body `alignItems:'stretch'` + card `marginTop:'auto'`). It is a **single growable A4 page** (`#print-mount`, `minHeight:1123px`) — no per-hole pages (cap has no holes).
2. `generatePdf()` in `pdfGenerator.ts` rasterizes the page — **html-to-image desktop / html2canvas mobile** — and slices it across A4 pages by height if it overflows. jsPDF assembles the PDF (A4). Heavy libs are dynamic imports.
3. `deliverPdf()` — **DIRECT download**. iOS: POST to `POST /api/download-pdf` returning `application/octet-stream` + attachment (Safari previews `application/pdf` inline, so octet-stream forces Save to Files); desktop/Android: `<a download>` blob, `revokeObjectURL` delayed 4s. The "Generating…" button holds until the iOS download UI returns.
4. File: `KAMINOS-ChimneyCap-YYYY-MM-DD.pdf`.
...
Kept in sync with chase (cart flow + capture pipeline + `cameraRef.ts` are byte-identical; only PDF *content* differs — cap's `PdfReport` is a single growable A4 page sliced by height, no per-hole pages).

- **"Network issue" = a client-side connection drop, NOT a code bug.** A live add from the user's phone was captured *succeeding* in logs; `/api/add-to-cart` + CORS test healthy; failures appear in ~5–10s (the 5-retry `postAddToCartApi` window) all `Failed to fetch`; and it persisted through a full rollback of the recent cart code. No version can add with no network — the **tap-to-retry banner** is the recovery.
- **Tap-to-retry (graceful recovery, BUILT)**: terminal add/buy failure sets a `submitError` state (`App.tsx` → `Sidebar` → `CartRow`) showing a "Try again / Dismiss" banner instead of `alert()`. "Try again" re-runs the same config; the idempotent server reuses any variant already created. Auto-clears on `isSubmitting`.
- **Screenshot framing — camera, not pixel-crop.** Cart/buy capture uses `cameraActions.fitView()` (bounding-sphere fit, 18% margin) so any-size product fits the frame AND lands in Shopify's centered-square cart tile. **Do NOT re-add the `getImageData` content-crop / pad-to-square to the cart path** — heavy per-add work that strained phones; reverted. `cropCanvasToContent` runs ONLY for the PDF (`{ framed: true }`). `canvasToJpegUnderLimit` (downscale + quality step-down) is kept everywhere — it prevents the variant-image 413 that left the cap red-variant imageless (showing the default photo).
- **Camera (`cameraRef.ts`)**: added `fitView` + `snapshot`/`restore`. `CapViewer` Canvas `near:0.05, far:100` (was 0.1/2000) fixes near-plane zoom clipping. `fitView` does NOT touch near/far.
- **DO NOT re-add cart-open timing experiments** (image-aware pre-open wait, post-open re-inject burst, 7s waits, image-polling catch-up) — tried to kill the mobile flash, then reverted; they added latency without fixing the connection-drop issue. Cart opens when price is verified; image fills in via the post-open refresh.
- **PDF**: big hero (`HERO_MAX 640×350`); **Pricing & Summary pinned bottom-right** (spec-body `alignItems:'stretch'` + card `marginTop:'auto'`). iOS direct download via `POST /api/download-pdf` (octet-stream attachment) + the "Generating…" button holds until the iOS download UI returns.
- **Cap-only nits not yet done**: `reset/top/front` in `cameraRef.ts` still use chase's pivot height (0.04 vs the Canvas's 0.12); no failure `alert`/`pdfDebug` telemetry in the PDF; no shadow-root sticky-header modal fix; `variant-image.ts` upload filenames still say `chase-cover-`.
...
- **429 rate limiting**: Shopify storefront 429s are retried with a longer backoff (~3.5–4.5s) inside `addToCartWithRetry` / `continueCartPreparationUntilVerified` — but a SUSTAINED throttle window exits early (3 hits in the main loop / 2 in the continuation, ~15–20s) with `rateLimited: true` and the friendly "store is busy" message, instead of grinding the full ~45s budget and throwing the misleading generic "could not confirm / refresh" error.
- **Throttle-aware fetches (June 13, 2026, ported from chase)**: bot protection answers **429 or 430** (`isThrottleStatus`). `fetchRenderedSections` and `fetchCartState` now return `{sections|cart, status}` so callers distinguish a rate-limited fetch from a not-ready one (a 429 challenge page is HTML, which the old code silently nulled). `waitForUsableRenderedSections` bails after 2 consecutive throttle statuses (`rateLimited: true`); the pre-open branch does NOT open an empty drawer when `rateLimited` (honest "item added — use the cart icon/page" alert instead); after a 200 add, 3 throttled `fetchCartState` reads exit with "your item may already be in the cart" (prevents the duplicate-add trap); catch-up stretches to 5s when throttled; `emitCartDebug('storefront-rate-limited', …)` once per flow; footprint trims (price-wait 1500→2000ms, confirm 1200→1500ms, section delay 1200→1500ms, restore fallback spaced 250ms, redundant pre-fetch deleted). None of this clears an already-flagged IP — that's Shopify-side (top-level cart-page visit / IP quiet time / different network). (The June 2026 unified-loop rewrite had lost this — only 422 was retried — so Shopify's raw bot-challenge text leaked into the error alert during a stress test; restored June 13, 2026.) HTML challenge-page bodies are never used as error messages, and `formatCheckoutErrorMessage` maps any "too many/rate limit/throttle/429" text to a friendly "store is busy" message. Shopify's per-IP limit was reproduced live at ~46 rapid adds in 40s; mobile carrier CGNAT (shared IPs) lowers the effective threshold.
...
- **`/api/add-to-cart` network resilience (June 13, 2026, evening)**: `postAddToCartApi` retries **5×** with a `600ms→3.5s` backoff (`getAddToCartApiRetryDelayMs`), spanning ~12s of wall-clock for fast-rejecting network errors. Raised from 3 attempts / ~3s after a user hit "network issue" on a **fresh device on mobile data** — verified server-side healthy (direct POST of the exact powder-coat config returned 200 with correct price; runtime logs showed only clean invocations), so the failure was a transient mobile-data drop (tower handoff / weak signal) longer than the old 3s give-up window; no variant was created. Timeouts are still capped at 2 attempts (`timeoutRetryExhausted`), so only fast network rejects use all 5. A genuine full connection drop on the client still can't be fixed in code (the honest "network issue — check your connection" message remains).
...
- **Mobile sticky-viewer z-index lowered 100 → 1 (June 13, 2026)**: on mobile the `.viewport` goes `position: sticky; top: 0` so the 3D viewer pins while the sidebar scrolls; its `z-index: 100` let the pinned viewer paint OVER the Shopify theme's open search dropdown (the reported bug — only surfaced on the cap, though the code is byte-identical to chase). Theme header is `z-index: 1000`, sidebar is non-positioned, so `z-index: 1` keeps the viewer above the sidebar but below all theme chrome; internal canvas/controls layering unaffected. Theme-integration issue — verify on the live Shopify embed, not the standalone dev preview.
- **Modal scroll-lock + PDF color name (June 13, 2026, ported from chase)**: the PDF preview and RAL color-picker modals no longer let the store page scroll behind them — `overscroll-behavior: contain` on their scroll containers + an App.tsx effect that locks `document` overflow while `pdfOpen || ralOpen` (restored on close; scoped to these two modals). The PDF's Powder Coat Color now shows the RAL name + code ("Ruby Red (RAL 3002)") via `ralLabel()` (reuses `RAL_COLORS`), with the hex on a small line below.
- **Pre-open section ceiling raised to 30s (June 13, 2026, evening, ported from chase)**: the drawer must open WITH the item already rendered. For a brand-new variant, Shopify's server-rendered cart-drawer shows the line at $0 until the variant price propagates (measured up to ~26s); an 8s/12s ceiling flashed empty then filled via catch-up ~2s later. `CART-SECTIONS` `maxWaitMs` is now 30000ms with the "Opening your cart" overlay step + reassuring note covering it. Safe because the throttle-bail means the big budget is only spent on genuine propagation lag (a blocked IP bails in ~3s); catch-up remains the >30s safety net. (Cap has no drag-to-move feature, so the chase "Drag-to-move PDF" snapshot fix does not apply here.)
- **Pre-open section wait raised 3.5s → 8s + stress hardening (June 13, 2026, ported from chase)**: the drawer prefers opening WITH the item already rendered (open-stale-then-pop-in was jarring); the post-open catch-up loop remains the safety net beyond 8s. Same port also brought the 429-retry restoration (see "429 rate limiting") and the screenshot capture/upload retries (see "Cart image").
- **Premature-open guard (June 2026, ported from chase)**: while an add/buy is in flight, `startPrematureCartOpenGuard` (350ms interval) force-closes any cart drawer that opens before the flow's own open step (`forceCloseCartUi` + scroll-lock release). The theme header cart icon stays clickable above the progress overlay, and the submission-time z-chain raise paints the configurator OVER an early-opened drawer — producing a half-buried stale drawer. The pre-open section injection also falls back to `applySectionUpdatesPreservingCartUi` if a drawer is somehow open at that moment. Guard stops right before `tryOpenCartUi()` (cart), before the checkout redirect (buy), and in both `finally` blocks.
- **Buy Now cart snapshot/restore (June 2026, ported from chase)**: see the **Buy Now Flow** section — customers no longer lose their other cart items when using Buy with Shop on the configurator.
